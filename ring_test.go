...
func TestRingSend(t *testing.T) {
	fmt.Println("TestRingSend creates 1000 servers and connects them logically in the form of a ring.")
	fmt.Println("The first server sends 1000 messages and confirms that it receives all the messages.")
	...
	for i := 0; i < serverCount; i += 1 {
		s, err := NewWithConfig(i, "127.0.0.1", 5481+i, &conf)
		if err != nil {
